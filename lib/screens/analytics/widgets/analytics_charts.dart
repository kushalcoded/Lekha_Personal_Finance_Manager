  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: LayoutBuilder(
        builder: (context, box) {
          final axisLabels = [
            for (final d in data) DateFormat('MMM').format(d.month),
          ];
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (value, meta) {
                      ...
                      if (index < 0 ||
                          index >= data.length ||
                          !axisLabelShown(index, data.length, step)) {
                        return const SizedBox.shrink();
                      }
                      return Padding(
                        padding: const EdgeInsets.only(top: 8),
                        child: Text(axisLabels[index], style: labelStyle),
                      );
                    },
                  ),
                ),
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: LayoutBuilder(
        builder: (context, box) {
          final axisLabels = [
            for (final p in points) _trendLabel(p.date, period),
          ];
          final labelStyle = theme.textTheme.labelSmall?.copyWith(
            color: colorScheme.onSurfaceVariant,
            fontWeight: FontWeight.w600,
                ...
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (value, meta) {
                      ...
                      if (index < 0 ||
                          index >= points.length ||
                          !axisLabelShown(index, points.length, step)) {
                        return const SizedBox.shrink();
                      }
...
String _trendLabel(DateTime date, String period) => switch (period) {
  'week' => DateFormat('E').format(date),
  'month' => DateFormat('MMM d').format(date),
  _ => DateFormat('MMM').format(date),
};
